	...
	for _, ns := range []string{"", "editor"} {
		t.Run("namespace="+ns, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				data, _ := json.Marshal(map[string]any{"type": "tool-call", "toolCallId": "new", "toolName": namespaceToolName(ns, "patch"), "input": map[string]any{"input": input}})
			...
			check(body["output"].([]any)[0].(map[string]any))
			seen := map[string]int{}
